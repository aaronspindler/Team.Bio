...
class TestCompanyViews(TestCase):
    ...
    def test_remove_user_login_required_post(self):
        self.client.logout()
        response = self.client.post(reverse('remove_user', kwargs={'email_prefix': self.company_user.email_prefix}), follow=True)
        self.assertEqual(response.status_code, 200)
    ...
    def test_remove_user_login_required_get(self):
        self.client.logout()
        response = self.client.get(reverse('remove_user', kwargs={'email_prefix': self.company_user.email_prefix}), follow=True)
        self.assertEqual(response.status_code, 200)
        self.assertTemplateNotUsed(response, "companies/remove_user.html")
        self.assertTemplateUsed(response, "account/login.html")
    ...
    def test_remove_user_is_a_company_owner(self):
        ...
        response = self.client.get(reverse('remove_user', kwargs={'email_prefix': self.user.email_prefix}), follow=True)
        self.assertEqual(response.status_code, 404)
        self.assertTemplateNotUsed(response, "companies/remove_user.html")
        self.assertTemplateUsed(response, "404.html")
        self.assertFalse(User.objects.filter(is_active=False).exists())
        ...
        # POST
        self.assertFalse(User.objects.filter(is_active=False).exists())
        response = self.client.post(reverse('remove_user', kwargs={'email_prefix': self.user.email_prefix}), follow=True)
        self.assertEqual(response.status_code, 404)
        self.assertTemplateNotUsed(response, "companies/remove_user.html")
        self.assertTemplateUsed(response, "404.html")
        self.assertFalse(User.objects.filter(is_active=False).exists())
    ...
    def test_remove_user_not_in_company(self):
        # GET
        self.assertFalse(User.objects.filter(is_active=False).exists())
        response = self.client.get(reverse('remove_user', kwargs={'email_prefix': self.different_company_user.email_prefix}), follow=True)
        self.assertEqual(response.status_code, 404)
        self.assertTemplateNotUsed(response, "companies/remove_user.html")
        self.assertTemplateUsed(response, "404.html")
        self.assertFalse(User.objects.filter(is_active=False).exists())
        ...
        # POST
        self.assertFalse(User.objects.filter(is_active=False).exists())
        response = self.client.post(reverse('remove_user', kwargs={'email_prefix': self.different_company_user.email_prefix}), follow=True)
        self.assertEqual(response.status_code, 404)
        self.assertTemplateNotUsed(response, "companies/remove_user.html")
        self.assertTemplateUsed(response, "404.html")
        self.assertFalse(User.objects.filter(is_active=False).exists())
